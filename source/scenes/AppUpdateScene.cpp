...
namespace dsu::scenes
{
    AppUpdateScene::AppUpdateScene()
    {
        ...
        _updateView = new UpdateView("Checking for updates to Stellar Updater...");
    ...
    void AppUpdateScene::render(SDL_Rect rect, double dTime)
    {
        if (_appVersionRequest == NULL)
        {
            _appVersionRequest = new WebRequest("https://api.github.com/repos/SpaceNX/Stellar-Updater/releases");
    ...
    void AppUpdateScene::_onCompleted(WebRequest *request)
    {
        if (request == _appVersionRequest)
        {
            ...
            else
            {
                ...
                _updateView->setText("Getting the latest version of Stellar Updater...");
        ...
        {
            romfsExit();
            FileManager::writeFile("Stellar-Updater.nro", request->response.rawResponseBody);
            _showStatus("Stellar Updater has been updated to version " + _appVersionRequest->response.rawResponseBody + "!", "Please restart the app.");
        }
    }
