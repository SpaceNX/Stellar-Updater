...
namespace dsu::scenes
{
    PackageSelectScene::PackageSelectScene()
    {
        ...
        _headerView = new HeaderView("Stellar Updater", true);
        _headerView->frame = {0, 0, 1280, 88};

        _updateView = new UpdateView("Checking for updates to Stellar...");
        ...
        _installRowView = new ListRowView("Install Latest Stellar", "", SUBTITLE);
        ...
        _ignoreConfigsAlertView = new AlertView("Ignore Config Files?", "Would you like for Stellar Updater to ignore config\nfiles? This will prevent Stellar Updater from overwriting\nall config files except for Hekate's main config file, and save the\nignored files for next time as well.", buttons);
    ...
    PackageSelectScene::~PackageSelectScene()
    {
        ...
        if (_StellarVersionRequest != NULL)
            delete _StellarVersionRequest;
    }
    ...
    void PackageSelectScene::render(SDL_Rect rect, double dTime)
    {
        if (_StellarVersionRequest == NULL)
        {
            _StellarVersionRequest = new WebRequest("https://api.github.com/repos/SpaceNX/Stellar/releases");
            SceneDirector::currentSceneDirector->render();
            SessionManager::makeRequest(_StellarVersionRequest);
        }
    ...
    void PackageSelectScene::_showPackageSelectViews(std::string StellarVersion)
    {
        ...
        if (version.compare(StellarVersion) == 0)
        {
            _installRowView->setPrimaryText("Reinstall Stellar");
        }
        else
        {
            _installRowView->setPrimaryText("Install Latest Stellar");
        }

        if (version == "" || version.compare(StellarVersion) == 0)
        {
            _installRowView->setSecondaryText("Latest Version is " + StellarVersion);
        }
        else
        {
            _installRowView->setSecondaryText("You currently have version " + version + " installed, and the latest version is " + StellarVersion + ".");
        }
    ...
    void PackageSelectScene::_onAlertViewDismiss(ModalView *view, bool success)
    {
        if (success && _ignoreConfigsAlertView->getSelectedOption() == 0)
        {
            ...
            files.push_back("sdmc:/switch/Stellar-Toolbox/config.json");
            files.push_back("sdmc:/switch/Stellar-Updater/internal.db");
            files.push_back("sdmc:/switch/Stellar-Updater/settings.cfg");
